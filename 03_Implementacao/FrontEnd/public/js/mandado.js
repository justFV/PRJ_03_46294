...
async function filtrar(vsearch) {
    ...
    if (proc == 'all' && pessoa == 'all' && tema == '0' && palChave == '0') {
        ...
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/search/" + proc + "/" + pessoa + "/" + tema + "/" + palChave;
        console.log(urlBase);
        ...
        await fetch(myRequest).then(async function (response) {
            if (!response.ok) {
                ...
                Mandados = await response.json();
                let tabela = '<table class="table">' +
                    '<tr>' +
                    '<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Referência Documental</th>' +
                    '<th scope="col">Registro</th><th scope="col">Resumo</th>' +
                    '<th scope="col">Pessoas<br>Citadas</th><th scope="col">Requerente</th>' +
                    '<th scope="col">Palavra<br>Chave</th><th scope="col">Tema</th><th>Ano Mandado</th>' +
                    '</tr>';
                conta = 0;
                for (const Mandado of Mandados) {
                    tabela += `<tr>
                                    ...
                                    <td scope="row">` + Mandado.referencia_documental + `</td>
                                    <td scope="row">` + Mandado.registro + `</td>
                                    <td scope="row">` + Mandado.resumo + `</td>
                                    <td scope="row">` + Mandado.pescita + `</td>
                                    <td scope="row">` + Mandado.pesreq + `</td>
                                    <td scope="row">` + Mandado.palcha + `</td>
                                    <td scope="row">` + Mandado.tema + `</td>
                ...
                tabela += `</table>`;
                document.getElementById("TableMandado").innerHTML = tabela;
            }
        });
    }
}
...
/************************* Mandado *************************** */
/************************ Lista todas os Mandados ****************************** */
async function getAllMandados(pageNum, rowsPage) {
    ...
    urlBase = rootUrl + "/CircPeticionario/webresources/mandado/all/" + pageNum + "/" + rowsPage;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    ...
    await fetch(myRequest).then(async function (response) {
        if (!response.ok) {
            console.log("Erro ao devolver a lista de mandados");
        } else {
            let objMan = await response.json();
            let Mandados = objMan.allMandados;
            let tabela = '<table class="table">' +
                '<tr>' +
                '<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Referência Documental</th>' +
                '<th scope="col">Registro</th><th scope="col">Resumo</th>' +
            ...
            let numPage = Math.ceil(objMan.rowcount / rowsPage);
            if (pageNum != 1) {
                tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllMandados(1,` + rowsPage + `)" style="color: white;">Inicio</a></span></li>`;
                if (pageNum > 5) {
            ...
            for (let i = 1; i <= numPage; i++) {
                if (i == pageNum) {
                    ...
                    if (i > pageNum - 5 && i < pageNum + 5) {
                        tabela += ` <li class="page-item"><a class="page-link" href="javascript:getAllMandados(` + i + `,` + rowsPage + `)">` + i + `</a></li>`;
                    }
                }
            }
            if (pageNum != numPage) {
                ...
                tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllMandados(` + numPage + `,` + rowsPage + `)" style="color: white;">Fim</a></span></li>`;
            }
            tabela += `</ul> `;
            document.getElementById("TableMandado").innerHTML = tabela;
        }
...
async function search(v_search) {
    if (v_search == null || v_search == '') {
        ...
        const tema = document.getElementById("selFilTema").value;
        const palChave = document.getElementById("selFilPalCha").value;
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/search/" + v_search + "/" + pessoa + "/" + tema + "/" + palChave;
...
/************************ devolve a Mandado pedido ****************************** */
async function getMandado(manId) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/record/" + manId;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    ...
    await fetch(myRequest).then(async function (response) {
        if (!response.ok) {
            ...
            document.getElementById("manObs").value = mandado.resumo;
            document.getElementById("anoProv").value = mandado.ano;
            document.getElementById("destQuemEnv").value = mandado.nome_quem_envia;
            document.getElementById("destMan").value = mandado.mandado;
            document.getElementById("datProv").value = mandado.data;
            document.getElementById("folioPagina").value = mandado.folio_pagina;
            document.getElementById("IdMan").innerHTML = 'Mandado Nº ' + manId;
            if (mandado.ano_post == "Y") {
                document.getElementById("anoPos").checked = true;
            } else {
                document.getElementById("anoPos").checked = false;
            }
            if (mandado.ano_post == "N") {
                document.getElementById("anoAnt").checked = true;
            } else {
                document.getElementById("anoAnt").checked = false;
            }
            getRefDoc(manId, mandado.referencia_documental);
            listRefDoc('');
            listRegistroAdd(mandado.registro, '');
            listSecEstado(manId);
            listProvocacao(manId);
            listMandado(manId);
            listResposta(manId);
            listConsulta(manId);
            listUltramar(manId);
            listPesCitada(manId);
            listPalavraChave(manId);
            listRequerente(manId);
            listTema(manId);
            listTermo(manId);
            listFreguesia(manId);
            listComarca(manId);
...
async function listSecEstado(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_mandado/allMandado/" + v_id;
    let texto = "";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectSecEstado(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    let texto = "";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listPesCitada(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_mandado/allMandado/" + v_id + "/all";
    let texto = "";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectPesCitada(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    let texto = "";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listPalavraChave(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_mandado/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectPalChave(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listRequerente(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_mandado/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectRequerente(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listTema(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_mandado/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectTema(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listTermo(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_mandado/allMandados/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectTermo(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listFreguesia(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_mandado/allMandados/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectFreguesia(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listComarca(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_mandado/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectComarca(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listCapitania(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_mandado/allMandados/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectCapitania(v_id, v_nomePesq) {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_mandado/notInMandado/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listProvocacao(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectProvocacao(v_id, v_nomePesq) {
    ...
    if (v_nomePesq == 'all' &&
        ...
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/notInMandado/" + v_id + "/" + v_nomePesq;
        var myHeaders = new Headers();
        myHeaders.append("token", await getToken());
...
async function listMandado(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_mandado/allRecente/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectMandado(v_id, v_nomePesq) {
    ...
    if (v_nomePesq == 'all' &&
        ...
        var json = JSON.stringify(objResp);
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_mandado/notInNovo/" + v_id + "/" + v_nomePesq;
        var myHeaders = new Headers();
        myHeaders.append("token", await getToken());
        ...
        await fetch(myRequest).then(async function (response) {
            if (!response.ok) {
                ...
                for (const listMandado of listMandados) {
                    ...
                    tableMandado += '<tr><td><a href="javascript:addMandadoMan(' + v_id + ',' + listMandado.id + ')"><b>Nº:' + listMandado.id + '</b>-' + listMandado.resumo + ' (' + listMandado.ano + ')</a></td>' +
...
async function listConsulta(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectConsulta(v_id, v_nomePesq) {
    ...
    if (v_nomePesq == 'all' &&
        ...
        var json = JSON.stringify(objResp);
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/notInMandado/" + v_id + "/" + v_nomePesq;
        var myHeaders = new Headers();
        myHeaders.append("token", await getToken());
...
async function listUltramar(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_ultramar/allUltramar/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectUltramar(v_id, v_nomePesq) {
    ...
    if (v_nomePesq == 'all' &&
        ...
        var json = JSON.stringify(objResp);
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_ultramar/notInUltramar/" + v_id + "/" + v_nomePesq;
        var myHeaders = new Headers();
        myHeaders.append("token", await getToken());
...
async function listResposta(v_id) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_resposta/allMandado/" + v_id + "/all";
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function selectResposta(v_id, v_nomePesq) {
    ...
    if (v_nomePesq == 'all' &&
        ...
        var json = JSON.stringify(objResp);
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_resposta/notInMandado/" + v_id + "/" + v_nomePesq;
        var myHeaders = new Headers();
        myHeaders.append("token", await getToken());
...
function limparFiltro() {
    ...
    selectProvocacao(sessionStorage.getItem("v_idMan"), document.getElementById('procProvocacao').value);
}
...
function limparFiltroMan() {
    ...
    selectMandado(sessionStorage.getItem("v_idMan"), document.getElementById('procMandado').value);
}
...
function limparFiltroCon() {
    ...
    selectConsulta(sessionStorage.getItem("v_idMan"), document.getElementById('procConsulta').value);
}
...
function limparFiltroUltra() {
    ...
    selectUltramar(sessionStorage.getItem("v_idMan"), document.getElementById('procUltramar').value);
}
...
function limparFiltroRes() {
    ...
    selectResposta(sessionStorage.getItem("v_idMan"), document.getElementById('procResposta').value);
}
...
async function salvar() {
    ...
    if (document.getElementById("manObs").value == "") {
        errForm += "Resumo é obrigatório.<br>";
    }
    if (document.getElementById("registroAddValue").value == "") {
        errForm += "Registo é obrigatório.<br>";
    }
    if (document.getElementById("datProv").value == "") {
        errForm += "Data é obrigatória.<br>";
    }
    if (document.getElementById("anoProv").value == "") {
        errForm += "Ano é obrigatório.<br>";
    }
    if (document.getElementById("destMan").value == "") {
        errForm += "Mandado é obrigatório.<br>";
    }

    if (document.getElementById("destQuemEnv").value == "") {
    ...
    if (errForm != "") {
        ...
        let v_idMan = sessionStorage.getItem("v_idMan");
        let v_anoPos = '';
        ...
        var objProv = {
            ...
            resumo: document.getElementById("manObs").value,
            mandado: document.getElementById("destMan").value,
            referencia_documental: document.getElementById("ref_docValue").value,
            nome_quem_envia: document.getElementById("destQuemEnv").value,
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/update/" + v_idMan,
            type: "PUT",
            headers: { "token": await getToken() },
            data: json,
            dataType: "json",
            success: function (result) {
                sessionStorage.setItem("v_idMan", result);
                Swal.fire({
                    icon: "success",
                    title: "Mandado Guardado.",
                    showConfirmButton: false,
                    timer: 1500,
                }).then(function () {
                    window.location = "mandado.html";
                });
...
// função de criação de nova Mandado
async function addMandado(v_id) {
    var errForm = "";
    ...
    if (errForm != "") {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/insert",
            type: "POST",
            headers: { "token": await getToken() },
            data: json,
            dataType: "json",
...
async function addSecEstado(v_prov_id, v_secEstado_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
async function delSecEstado(v_prov_id) {
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_mandado/delete/" + v_secEstado_id + "/" + v_prov_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
async function addPesCitada(v_man_id, v_pesCitada_id) {
    //Se não houver erro no preenchimento então chama o serviço de registo
    var objResp = {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
async function delPesCitada(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_mandado/delete/" + v_pesCitada_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
async function addPalChave(v_man_id, v_dpalChave_id) {
    //Se não houver erro no preenchimento então chama o serviço de registo
    var objResp = {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
async function delPalChave(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_mandado/delete/" + v_man_id + "/" + v_pchave_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
async function addRequerente(v_man_id, v_requerente_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Requerente ************************* */
async function delRequerente(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_mandado/delete/" + v_requerente_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Tema ***************************** */
/********************* Adicionar Tema ************************* */
async function addTema(v_man_id, v_tema_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Tema ************************* */
async function delTema(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_mandado/delete/" + v_tema_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Termo ***************************** */
/********************* Adicionar Termo ************************* */
async function addTermo(v_man_id, v_termo_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Termo ************************* */
async function delTermo(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_mandado/delete/" + v_termo_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Freguesia ***************************** */
/********************* Adicionar Freguesia ************************* */
async function addFreguesia(v_man_id, v_freg_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Freguesia ************************* */
async function delFreguesia(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_mandado/delete/" + v_freg_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Comarca ***************************** */
/********************* Adicionar Comarca ************************* */
async function addComarca(v_man_id, v_coma_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Comarca ************************* */
async function delComarca(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_mandado/delete/" + v_coma_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Capitania ***************************** */
/********************* Adicionar Capitania ************************* */
async function addCapitania(v_man_id, v_cap_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Capitania ************************* */
async function delCapitania(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_mandado/delete/" + v_coma_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
async function addProvocacao(v_man_id, v_provocacao_id) {
...
/********************* Retirar Provocação ************************* */
async function delProvocacao(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/delete/" + v_man_id + "/" + v_procacao_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Mandado ***************************** */
/********************* Adicionar Mandado ************************* */
async function addMandadoMan(v_man_id, v_mandado_id) {

    var objResp = {
        mandado_mais_antigo: v_mandado_id,
        mandado_mais_recente: v_man_id
    };
    var exit = "no";
    var json = JSON.stringify(objResp);
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_mandado/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Mandado ************************* */
async function delMandado(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_mandado/delete/" + v_mandado_id + "/" + v_man_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Consulta ***************************** */
/********************* Adicionar Consulta ************************* */
async function addConsulta(v_man_id, v_consulta_id) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/insert",
        type: "POST",
        headers: { "token": await getToken() },
        data: json,
        dataType: "json",
...
/********************* Retirar Consulta ************************* */
async function delConsulta(v_man_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/delete/" + v_man_id + "/" + v_consulta_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Ultramar ***************************** */
/********************* Adicionar Ultramar ************************* */
async function addUltramar(v_mans_id, v_ultramar_id) {
...
/********************* Retirar Ultramar ************************* */
async function delUltramar(v_mans_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
    ...
    for (let element of elements) {
        ...
        myHeaders.append("token", await getToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_ultramar/delete/" + v_mans_id + "/" + v_ultramar_id,
            type: "DELETE",
            headers: { "token": await getToken() },
            data: "",
            dataType: "json"
...
/************************** Resposta ***************************** */
/********************* Adicionar Resposta ************************* */
async function addResposta(v_mandado_id, v_resp_id) {
...
/********************* Retirar Resposta ************************* */
async function delResposta(v_mandado_id) {
    // vai percorrer todos os item da página que utilizem a class checked
    // Se checked e clicar eliminar remove da lista os selecionados
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                Swal.fire({
                    ...
                    let v_tipoAdd = sessionStorage.getItem("v_AddPes");
                    if (v_tipoAdd == 'requerente') {
                        addRequerente(sessionStorage.getItem("v_idMan"), pessoaId);
                    } else {
                        if (v_tipoAdd == 'pesCitada') {
                            addPesCitada(sessionStorage.getItem("v_idMan"), pessoaId)
                        }
                    }
                });
            },
...
function delMandado() {
    Swal.fire({
        icon: "error",
        title: "Tem a certeza que quer eliminar este mandado?",
        ...
        if (result.isConfirmed) {
            ...
            myHeaders.append("token", await getToken());
            $.ajax({
                url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/delete/" + sessionStorage.getItem("v_idMan"),
                type: "DELETE",
                headers: { "token": await getToken() },
                dataType: "json",
                success: function (result) {
                    Swal.fire({
                        icon: "success",
                        title: "Mandado eliminado com sucesso.",
                        showConfirmButton: false,
                        timer: 1500,
                    }).then(function () {
                        window.location = "mandado.html";
                ...
                error: function (err) {
                    ...
                    Swal.fire({
                        icon: "error",
                        title: "Erro ao elimiar o mandado.",
                        text: "Este mandado encontra-se associada a um registo",
                        showConfirmButton: false,
                        timer: 1500,
                    });
                }
