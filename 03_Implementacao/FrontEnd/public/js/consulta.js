function getAno(v_valor) {
  document.getElementById("anoCons").value = v_valor.substr(6);
}
...
async function filtrar(vsearch) {
    ...
    if (proc == 'all' && pessoa == 'all' && tema == '0' && palChave == '0') {
        ...
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/search/" + proc + "/" + pessoa + "/" + tema + "/" + palChave;
        console.log(urlBase);
        ...
        await fetch(myRequest).then(async function (response) {
            if (!response.ok) {
                ...
                '<th scope="col">Referência Documental</th><th scope="col">Sumula</th><th scope="col">Pessoas<br>Citadas</th><th scope="col">Requerente</th>' +
                '<th scope="col">Palavra<br>Chave</th><th scope="col">Tema</th><th>Ano consulta</th>' +
                '</tr>';
                conta = 0;
                for (const Consulta of Consultas) {
                    tabela += `<tr>
                                  <td scope="row"><i class="fa fa-pen-fancy" style="cursor:pointer;" onclick="editConsulta(`+ Consulta.id + `)"></i></td>
                                  <td scope="row">` + Consulta.pk_consulta + `</td>
                                  <td scope="row">` + Consulta.resumo + `</td>
                                  <td scope="row">` + Consulta.registro + `</td>
                                  <td scope="row">` + Consulta.referencia_documental + `</td>
                                  <td scope="row">` + Consulta.sumula + `</td>
                                  <td scope="row">` + Consulta.pescita + `</td>
                                  <td scope="row">` + Consulta.pesreq + `</td>
                                  <td scope="row">` + Consulta.palcha + `</td>
                                  <td scope="row">` + Consulta.tema + `</td>
                ...
                tabela += `</table>`;
                document.getElementById("TableConsulta").innerHTML = tabela;
            }
        });
    }
}
...
async function getAllConsultas(pageNum, rowsPage) {
  ...
  urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/all/" + pageNum + "/" + rowsPage;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      console.log("Erro ao devolver a lista de consultas");
    } else {
      let objCons = await response.json();
      let Consultas = objCons.allConsulta;
      let tabela = '<table class="table">' +
        '<tr>' +
        '<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Resumo</th><th scope="col">Registro</th>' +
        '<th scope="col">Referência Documental</th><th scope="col">Sumula</th><th scope="col">Pessoas<br>Citadas</th><th scope="col">Requerente</th>' +
      ...
      let numPage = Math.ceil(objCons.rowcount / rowsPage);
      if (pageNum != 1) {
          tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllConsultas(1,` + rowsPage + `)" style="color: white;">Inicio</a></span></li>`;
          if (pageNum > 5) {
      ...
      for (let i = 1; i <= numPage; i++) {
          if (i == pageNum) {
              ...
              if (i > pageNum - 5 && i < pageNum + 5) {
                  tabela += ` <li class="page-item"><a class="page-link" href="javascript:getAllConsultas(` + i + `,` + rowsPage + `)">` + i + `</a></li>`;
              }
          }
      }
      if (pageNum != numPage) {
          ...
          tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllConsultas(` + numPage + `,` + rowsPage + `)" style="color: white;">Fim</a></span></li>`;
      }
      tabela += `</ul> `;
      document.getElementById("TableConsulta").innerHTML = tabela;
    }
...
async function search(v_search) {
  if (v_search == null || v_search == '') {
    ...
    const tema = document.getElementById("selFilTema").value;
    const palChave = document.getElementById("selFilPalCha").value;
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/search/" + v_search + "/" + pessoa + "/" + tema + "/" + palChave;
...
/************************ devolve a consulta pedida ****************************** */
async function getConsulta(v_consId) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/record/" + v_consId;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      document.getElementById("consObs").value = consulta.resumo;
      document.getElementById("anoCons").value = consulta.ano_consulta;
      document.getElementById("parReal").value = consulta.parecer_regio;
      document.getElementById("sumula").value = consulta.sumula;
      document.getElementById("datCons").value = consulta.data_consulta;
      document.getElementById("datParReal").value = consulta.data_parecer_regio;
      document.getElementById("folioPagina").value = consulta.folio_pagina;
      document.getElementById("IdCons").innerHTML = 'Consulta Nº '+v_consId;
      if (consulta.anoPost == "Y") {
        document.getElementById("anoPos").checked = true;
      } else {
        document.getElementById("anoPos").checked = false;
      }
      if (consulta.anoPost == "N") {
        document.getElementById("anoAnt").checked = true;
      } else {
        document.getElementById("anoAnt").checked = false;
      }
      getRefDoc(v_consId, consulta.referencia_documental);
      listRefDoc('');
      listRegistroAdd(consulta.registro, '');
      listSecEstado(v_consId);
      listMandado(v_consId);
      listUltramar(v_consId);
      listTema(v_consId);
      listRequerente(v_consId);
      listPalavraChave(v_consId);
      listPesCitada(v_consId);
      listProvocacao(v_consId);
      listResposta(v_consId);
      listConsulta(v_consId);
      listTermo(v_consId);
      listFreguesia(v_consId);
      listComarca(v_consId);
      listCapitania(v_consId);
...
async function listSecEstado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_consulta/allConsulta/" + v_id;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectSecEstado(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listPesCitada(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_consulta/allConsulta/" + v_id;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectPesCitada(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listPalavraChave(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_consulta/allConsulta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectPalChave(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listRequerente(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_consulta/allConsulta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectRequerente(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listTema(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_consulta/allConsulta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectTema(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listTermo(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_consulta/allConsulta/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectTermo(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listFreguesia(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_consulta/allConsultas/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectFreguesia(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listComarca(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_consulta/allConsultas/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectComarca(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listCapitania(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_consulta/allConsultas/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectCapitania(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listProvocacao(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/allConsulta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectProvocacao(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/notInConsulta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listMandado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/allConsulta/" + v_id; /* + "/all"; */
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectMandado(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listConsulta(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_consulta/allRecente/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectConsulta(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_consulta/notInNova/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listUltramar(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_consulta/allConsulta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectUltramar(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_consulta/notInConsulta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listResposta(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/allConsulta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectResposta(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/notInConsulta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function salvar() {
  ...
  if (errForm != "") {
    ...
    let v_idCons = sessionStorage.getItem("v_idCons");
    let v_anoPos = '';
    ...
    var objCons = {
      data_consulta: document.getElementById("datCons").value,
      resumo: document.getElementById("consObs").value,
      parecer_regio: document.getElementById("parReal").value,
      sumula: document.getElementById("sumula").value,
      registro: document.getElementById("registroAddValue").value,
      ano_consulta: document.getElementById("anoCons").value,
      referencia_documental: document.getElementById("ref_docValue").value,
      data_parecer_regio: document.getElementById("datParReal").value,
      anoPost: v_anoPos,
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/update/" + v_idCons,
      type: "PUT",
      ...
      error: function (err) {
        ...
        Swal.fire({
          icon: "error",
          title: "Erro ao guardar Consulta.",
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
          window.location = "consulta.html";
...
// função de criação de nova consulta
async function addNovaConsulta(v_id) {
  var errForm = "";
  // validação do preenchimento dos campos
  if (document.getElementById("consObs").value == "") {
    errForm += "Resumo é obrigatório.<br>";
  ...
  if (errForm != "") {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/insert",
      type: "POST",
      headers: { "token": await getToken() },
      data: json,
      dataType: "json",
...
async function addSecEstado(v_prov_id, v_secEstado_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delSecEstado(v_prov_id) {
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_consulta/delete/" + v_secEstado_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addPesCitada(v_cons_id, v_pesCitada_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delPesCitada(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_consulta/delete/" + v_pesCitada_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addPalChave(v_cons_id, v_dpalChave_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delPalChave(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_consulta/delete/" + v_cons_id + "/" + v_pchave_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addRequerente(v_cons_id, v_requerente_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Requerente ************************* */
async function delRequerente(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_consulta/delete/" + v_requerente_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Tema ***************************** */
/********************* Adicionar Tema ************************* */
async function addTema(v_cons_id, v_tema_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Tema ************************* */
async function delTema(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_consulta/delete/" + v_tema_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Termo ***************************** */
/********************* Adicionar Termo ************************* */
async function addTermo(v_cons_id, v_termo_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Termo ************************* */
async function delTermo(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_consulta/delete/" + v_termo_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Freguesia ***************************** */
/********************* Adicionar Freguesia ************************* */
async function addFreguesia(v_cons_id, v_freg_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Freguesia ************************* */
async function delFreguesia(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_consulta/delete/" + v_freg_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Comarca ***************************** */
/********************* Adicionar Comarca ************************* */
async function addComarca(v_cons_id, v_coma_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Comarca ************************* */
async function delComarca(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_consulta/delete/" + v_coma_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Capitania ***************************** */
/********************* Adicionar Capitania ************************* */
async function addCapitania(v_cons_id, v_cap_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Capitania ************************* */
async function delCapitania(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_consulta/delete/" + v_coma_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addProvocacao(v_cons_id, v_provocacao_id) {
...
/********************* Retirar Provocação ************************* */
async function delProvocacao(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/delete/" + v_cons_id + "/" + v_procacao_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Mandado ***************************** */
/********************* Adicionar Mandado ************************* */
async function addMandado(v_cons_id, v_mandado_id) {
...
/********************* Retirar Mandado ************************* */
async function delMandado(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_consulta/delete/" + v_mandado_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Consulta ***************************** */
/********************* Adicionar Consulta ************************* */
async function addConsulta(v_cons_id, v_consulta_id) {

  var objResp = {
    consulta_mais_recente: v_cons_id,
    consulta_mais_antiga: v_consulta_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_consulta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Consulta ************************* */
async function delConsulta(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_consulta/delete/" + v_consulta_id + "/" + v_cons_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Ultramar ***************************** */
/********************* Adicionar Ultramar ************************* */
async function addUltramarRel(consulta, ultramar) {
...
/********************* Retirar Ultramar ************************* */
async function delUltramar(consulta) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_consulta/delete/" + v_ultramar_id + "/" + consulta,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Resposta ***************************** */
/********************* Adicionar Resposta ************************* */
async function addResposta(v_cons_id, v_resp_id) {
...
/********************* Retirar Resposta ************************* */
async function delResposta(v_cons_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/delete/" + v_cons_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
    ...
    $.ajax({
      ...
      success: function ( result) {
        ...
        for (let element of elementsMarEco) {
          console.log('Valor Mar Eco: '+element.attributes.value.value);
          var objMarEco = {
        ...
        Swal.fire({
          ...
          let v_tipoAdd = sessionStorage.getItem("v_AddPes");
          if (v_tipoAdd == 'requerente') {
            addRequerente(sessionStorage.getItem("v_idCons"), pessoaId);
          } else {
            if (v_tipoAdd == 'pesCitada') {
              addPesCitada(sessionStorage.getItem("v_idCons"), pessoaId)
            }
          }
        });
      },
...
function delCons() {
  Swal.fire({
    icon: "error",
    title: "Tem a certeza que quer eliminar esta consulta?",
    ...
    if (result.isConfirmed) {
      ...
      myHeaders.append("token", await getToken());
      $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/consulta/delete/" + sessionStorage.getItem("v_idCons"),
        type: "DELETE",
        headers: { "token": await getToken() },
        dataType: "json",
        success: function (result) {
          Swal.fire({
            icon: "success",
            title: "Consulta eliminada com sucesso.",
            showConfirmButton: false,
            timer: 1500,
          }).then(function () {
            window.location = "consulta.html";
        ...
        error: function (err) {
          ...
          Swal.fire({
            icon: "error",
            title: "Erro ao elimiar a consulta.",
            text: "Esta consulta encontra-se associada a um registo",
            showConfirmButton: false,
            timer: 1500,
          });
        }
