...
function getAno(v_valor) {
  document.getElementById("anoProv").value = v_valor.substr(6);
}
...
async function filtrar(vsearch) {
  ...
  if (proc == 'all' && pessoa == 'all' && tema == '0' && palChave == '0') {
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/provocacao/search/" + proc + "/" + pessoa + "/" + tema + "/" + palChave;
    console.log(urlBase);
    ...
    await fetch(myRequest).then(async function (response) {
      if (!response.ok) {
        ...
        Provocacoes = await response.json();
        let tabela = '<table class="table">' +
          '<tr>' +
          '<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Destinatário Autoridade Tratamento</th><th scope="col">Resumo</th>' +
          '<th scope="col">Referência Documental</th><th scope="col">Pessoa<br>Citadas</th><th scope="col">Requerente</th>' +
          '<th>Palavra<br>Chave</th><th>Tema</th><th>Ano Provocação</th>' +
          '</tr>';
        conta = 0;
        for (const Provocacao of Provocacoes) {
          tabela += `<tr>
                                  <td scope="row"><i class="fa fa-pen-fancy" style="cursor:pointer;" onclick="editProvocacao(`+ Provocacao.pk_provocacao + `)"></i></td>
                                  <td scope="row">` + Provocacao.pk_provocacao + `</td>
                                  <td scope="row">` + Provocacao.destinatario_autoridade_tratamento + `</td>
                                  <td scope="row">` + Provocacao.resumo + `</td>
                                  <td scope="row">` + Provocacao.referencia_documental + `</td>
                                  <td scope="row">` + Provocacao.pescita + `</td>
                                  <td scope="row">` + Provocacao.pesreq + `</td>
                                  <td scope="row">` + Provocacao.palcha + `</td>
                                  <td scope="row">` + Provocacao.tema + `</td>
        ...
        tabela += `</table>`;
        document.getElementById("TableProvocacao").innerHTML = tabela;
      }
    });
  }
}
...
/************************* Provocação *************************** */
/************************ Lista todas as procações ****************************** */
async function getAllprovocacoes(pageNum, rowsPage) {
  ...
  urlBase = rootUrl + "/CircPeticionario/webresources/provocacao/all/" + pageNum + "/" + rowsPage;
  myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let objProv = await response.json();
      let Provocacoes = objProv.allProvocacao;
      let tabela = '<table class="table">' +
        '<tr>' +
        '<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Destinatário Autoridade Tratamento</th><th scope="col">Resumo</th>' +
        '<th scope="col">Referência Documental</th><th scope="col">Pessoa<br>Citadas</th><th scope="col">Requerente</th>' +
      ...
      if (pageNum != 1) {
        tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllprovocacoes(1,` + rowsPage + `)" style="color: white;">Inicio</a></span></li>`;
        if(pageNum > 5 ){
      ...
      for (let i = 1; i <= numPage; i++) {
        if (i == pageNum) {
          ...
          if ( i > pageNum - 5  && i < pageNum + 5) {
            tabela += ` <li class="page-item"><a class="page-link" href="javascript:getAllprovocacoes(` + i + `,` + rowsPage + `)">` + i + `</a></li>`;
          } 
        }
      }
      if (pageNum != numPage) {
        ...
        tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllprovocacoes(` + numPage + `,` + rowsPage + `)" style="color: white;">Fim</a></span></li>`;
      }
      tabela += `</ul> `;
      document.getElementById("TableProvocacao").innerHTML = tabela;
    }
...
async function search(v_search) {
  if (v_search == null || v_search == '') {
    ...
    await
    getAllprovocacoes(v_pageNum, 10);
  } else {
    let pessoa = document.getElementById("filPes").value;
    if (pessoa == null || pessoa == '') {
      pessoa = "all";
    ...
    const tema = document.getElementById("selFilTema").value;
    const palChave = document.getElementById("selFilPalCha").value;
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/provocacao/search/" + v_search + "/" + pessoa + "/" + tema + "/" + palChave;
...
/************************ devolve a provocação pedida ****************************** */
async function getProvocacao(v_provId) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/provocacao/record/" + v_provId;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      document.getElementById("provObs").value = provocacao.resumo;
      document.getElementById("anoProv").value = provocacao.ano;
      document.getElementById("datProv").value = provocacao.data;
      document.getElementById("folioPagina").value = provocacao.folio_pagina;
      document.getElementById("IdProv").innerHTML = 'Provocação Nº ' + v_provId;
      if (provocacao.anoPost == "Y") {
        document.getElementById("anoPos").checked = true;
      } else {
        document.getElementById("anoPos").checked = false;
      }
      if (provocacao.anoPost == "N") {
        document.getElementById("anoAnt").checked = true;
      } else {
        document.getElementById("anoAnt").checked = false;
      }
      document.getElementById("destAut").value = provocacao.destinatario_autoridade_tratamento;
      getRefDoc(v_provId, provocacao.referencia_documental);
      listRefDoc('');
      listRegistroAdd(provocacao.registro, '');
      listSecEstado(v_provId);
      listConsulta(v_provId);
      listMandado(v_provId);
      listUltramar(v_provId);
      listProvocacao(v_provId);
      listTema(v_provId);
      listRequerente(v_provId);
      listPalavraChave(v_provId);
      listPesCitada(v_provId);
      listRemetente(v_provId);
      listTermo(v_provId);
      listFreguesia(v_provId);
      listComarca(v_provId);
      listCapitania(v_provId);
      listResposta(v_provId);
      getDestAuto(provocacao.destinatario_autoridade_tratamento);
...
/****************** Registros ******************* */
async function getAllRegistos(v_id, v_obj) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/registro/all";
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      console.log("Erro ao devolver a lista de registros");
    } else {
      let registos = await response.json();
      let checkbox = '<div id="registos">';
      let conta = 1;
      let selected = '';
      for (const registo of registos) {

        if (v_id == registo.pk_registro) {
        ...
        checkbox += '<div class="form-check"><input class="form-check-input" name="checkRegisto" type="radio" value="' + registo.pk_registro + '" id="checkRegisto' + registo.pk_registro + '" ' + selected + ' >' +
          '<label class="form-check-label" for="checkRegisto' + registo.pk_registro + '">' +
          registo.descricao +
          '</label></div>'
        if (conta == 4) {
          checkbox += '</div></div>';
...
async function getRefDocs(v_respId, v_refDocId) {
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      refDoc = await response.json();
      adicRefDoc(v_respId, refDoc.referencia + ' (Nº:' + refDoc.numero + ')');
    }
  })
}
...
async function listSecEstado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_provocacao/allProvocacao/" + v_id;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectSecEstado(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
/************************ devolve o Destinatário pedido ****************************** */
function getDestAuto(v_id) {
  v_destAuto = '<div class="row-cols-sm-1" style="margin-left: 40px;">';
  if (v_id == 'S') {
    v_destAuto += '<input class="form-check-input" name="destAut" type="radio" value="S" id="destAutS" checked="checked">';
  } else {
    v_destAuto += '<input class="form-check-input" name="destAut" type="radio" value="S" id="destAutS">';
  }
  v_destAuto += '<label class="form-check-label" for="destAutS">Secretário (Ilmo/Exmo)</label>' +
    '</div>' +
    '<div class="row-cols-sm-1" style="margin-left: 40px;">'
  if (v_id == 'R') {
    v_destAuto += '<input class="form-check-input" name="destAut" type="radio" value="R" id="destAuR" checked="checked">';
  } else {
    v_destAuto += '<input class="form-check-input" name="destAut" type="radio" value="R" id="destAutR">';
  }
  v_destAuto += '<label class="form-check-label" for="destAutR">Rei/Rainha (Snr./Snra.)</label>' +
    '</div>';

  document.getElementById("destAutoSR").innerHTML = v_destAuto;
...
async function listPesCitada(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_provocacao/allProvocacao/" + v_id + "/all";
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let tableDPesCitada = '<div class="row" ><div class="column listTabNome" align="left"><label for="tableListPesCitada" >Pessoas Citadas:</label>' +
...
async function selectPesCitada(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
/****************** Remetente ******************* */
/******************** Devolve lista dos Remetentes já introduzidos *************************** */
async function listRemetente(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_remetente_provocacao/allRemetente/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      console.log("Erro ao devolver a lista dos Remetentes");
    } else {
      let remetentes = await response.json();
      let tableRemetente = '<div class="row" ><div class="column listTabNome" align="left"><label for="tableRemetente" >Remetente:</label>' +
        '</div><div class="column listTabBotao" align="right"><a href="javascript:selRemetente(' + v_id + ')"><img src="../img/addLine.png" width="35"></a> ' +
        '<a href="javascript:delRemetente(' + v_id + ')"><img src="../img/delList.png" width="35"></a></div></div>' +
        '<table id="tableRemetente" class="listPopUp">';

      for (const remetente of remetentes) {

        tableRemetente += '<tr><td class="checkDel"><i class="checkDelRemetente far fa-circle" style="color:red;" onclick="checkDelRemetente(this);" value="' + remetente.id + '"></i></td><td>' + remetente.nome + '</td></tr>'


      }
      tableRemetente += `</table>`;
      document.getElementById("remetente").innerHTML = tableRemetente;
    }
  })
}

/*********************** Devolve lista dos remetentes que ainda não foram escolhidos ************************************** */
async function selectRemetente(v_id, v_nomePesq) {
  if (v_nomePesq == null) {
    v_nomePesq = 'all';
  }
  if (v_nomePesq == '') {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_remetente_provocacao/notInRemetente/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let listRemetentes = await response.json();
      let tableRemetente = '<table id="tableSelRemetente" class="ListPopUpModal">';
      let conta = 1;
      for (const listRemetente of listRemetentes) {

        tableRemetente += '<tr><td><a href="javascript:addRemetente(' + v_id + ',' + listRemetente.id + ')">' + listRemetente.nome + '</a></td>' +
          '<td> <span type="button" onclick="getPessoaInfo(' + listRemetente.id + ')">' +
          '<img src="../img/info.png" width="20px"></span></td></tr>'
      ...
      if (conta >= 10) {
        tableRemetente += '<tr><td>Esta pesquisa devolveu muitos valores.<br>Por afine a sua pesquisa para devolver menos valores.</td></tr>'
      }
      tableRemetente += `</table>`;
      document.getElementById("tabmodalRemetente").innerHTML = tableRemetente;
    }
  })
}
...
async function listPalavraChave(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectPalChave(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listRequerente(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectRequerente(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      for (const listRequerente of listRequerentes) {
        ...
        tableRequerente += '<tr><td><a href="javascript:addRequerente(' + v_id + ',' + listRequerente.id + ')">' + listRequerente.nome + '</td>' +
...
async function listTema(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let tableTemas = '<div class="row" ><div class="column listTabNome" align="left"><label for="tableTemas" >Tema:</label>' +
        '</div><div class="column listTabBotao" align="right"><a href="javascript:selTema(' + v_id + ',document.getElementById(`procTema`).value)"><img src="../img/addLine.png" width="35"></a> ' +
...
async function selectTema(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listTermo(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectTermo(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listFreguesia(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectFreguesia(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listComarca(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectComarca(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listCapitania(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectCapitania(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listProvocacao(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_provocacao/allProvAntiga/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectProvocacao(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_provocacao/notInProvocacaoAntiga/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function getProvocacaoInfo(provId) {
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      document.getElementById("anoProvInfo").innerHTML = provocacao.ano;
      document.getElementById("destAut").innerHTML = provocacao.destinatario_autoridade_tratamento;
      document.getElementById("datProvInfo").innerHTML = provocacao.data;
...
async function listMandado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectMandado(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function getMandadoInfo(provId) {

  $('#modalMandado').modal('hide');
  $('#infoMan').modal('show');

  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/mandado/recordInfo/" + provId;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      sessionStorage.setItem("v_idMan", provId);
...
async function listConsulta(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectConsulta(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function getConsultaInfo(consId) {
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      document.getElementById("anoConsInfo").innerHTML = consulta.ano_consulta;
      document.getElementById("parRealInfo").innerHTML = consulta.parecer_regio;
      document.getElementById("sumulaInfo").innerHTML = consulta.sumula;
      document.getElementById("datConsInfo").innerHTML = consulta.data_consulta;
      document.getElementById("datParRealInfo").innerHTML = consulta.data_parecer_regio;
      document.getElementById("consRefDocInfo").innerHTML = consulta.referencia_documentalDesc;
...
async function listUltramar(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_provocacao/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectUltramar(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_provocacao/notInProvocacao/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listResposta(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/allProvocacao/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectResposta(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    console.log(objResp)
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/notInProvocacao/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function getPessoaInfo(pesId) {
  ...
  if (v_tipoAdd == 'requerente') {
    ...
    if (v_tipoAdd == 'pesCitada') {
      ...
      if (v_tipoAdd == 'remetente') {
        $('#modalRemetente').modal('hide');
      }
    }
  }
  $('#infoPes').modal('show');
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      //document.getElementById("marPes").innerHTML = pessoa.marcador_economico_ocupacaoDesc;
...
function limparFiltro() {
  ...
  selectProvocacao(sessionStorage.getItem("v_idProv"), document.getElementById('procProvocacao').value);
}
...
function limparFiltroMan() {
  ...
  selectMandado(sessionStorage.getItem("v_idProv"), document.getElementById('procMandado').value);
}
...
function limparFiltroCon() {
  ...
  selectConsulta(sessionStorage.getItem("v_idProv"), document.getElementById('procConsulta').value);
}
...
function limparFiltroUltra() {
  ...
  selectUltramar(sessionStorage.getItem("v_idProv"), document.getElementById('procUltramar').value);
}
...
function limparFiltroRes() {
  ...
  selectResposta(sessionStorage.getItem("v_idProv"), document.getElementById('procResposta').value);
}
...
async function salvar() {
  ...
  var errForm = "";
  //Percorre a lista dos Destinatários para encontrar o selecionado
  var vDest = document.getElementsByName('destAut');
  var vDestValue = "0";
  for (i = 0; i < vDest.length; i++) {
    if (vDest[i].checked)
      vDestValue = vDest[i].value
  }
  // validação do preenchimento dos campos
  if (document.getElementById("provObs").value == "") {
    errForm += "Resumo é obrigatório.<br>";
  }
  if (document.getElementById("registroAddValue").value == "") {
    errForm += "Registo é obrigatório.<br>";
  }
  if (document.getElementById("anoProv").value == "") {
  ...
  if (errForm != "") {
    ...
    let v_idProv = sessionStorage.getItem("v_idProv");
    ...
    var objProv = {
      data: document.getElementById("datProv").value,
      resumo: document.getElementById("provObs").value,
      destinatario_autoridade_tratamento: vDestValue,
      referencia_documental: document.getElementById("ref_docValue").value,
      registro: document.getElementById("registroAddValue").value,
      ano: document.getElementById("anoProv").value,
      anoPost: v_anoPos,
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/provocacao/update/" + v_idProv,
      type: "PUT",
      headers: { "token": await getToken() },
      data: json,
      dataType: "json",
      success: function (result) {
        sessionStorage.setItem("v_idProv", result);
        Swal.fire({
          icon: "success",
          title: "Provocação guardada.",
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
          sessionStorage.setItem("v_idProv", result);
          window.location = "provocacao.html";
        });
      ...
      error: function (err) {
        ...
        Swal.fire({
          icon: "error",
          title: "Erro ao guardar provocação",
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
        });
...
// função de criação de nova Provocação
async function addNovaProvocacao() {
  var errForm = "";
  //Percorre a lista dos Destinatários para encontrar o selecionado
  var vDest = document.getElementsByName('destAut');
  var vDestValue = "0";
  ...
  if (errForm != "") {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/provocacao/insert",
      type: "POST",
      headers: { "token": await getToken() },
      data: json,
      dataType: "json",
      ...
      error: function (err) {
        ...
        Swal.fire({
          icon: "error",
          title: "Erro ao criar provocação",
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
          window.location = "provocacao.html";
...
async function addRefDoc() {
  ...
  if (errForm != "") {
    ...
    var objCons = {
      ...
      // tipo_referencia: document.getElementById("tipRefSel").value
    };
    var exit = "no";
...
async function addSecEstado(v_prov_id, v_secEstado_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delSecEstado(v_prov_id) {
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_provocacao/delete/" + v_secEstado_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addPesCitada(v_prov_id, v_pesCitada_id) {
  var objResp = {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delPesCitada(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pcitadas_provocacao/delete/" + v_pesCitada_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Remetente ***************************** */
/********************* Adicionar Remetente ************************* */
async function addRemetente(v_prov_id, v_remetente_id) {

  var objResp = {
    remetente: v_remetente_id,
    provocacao: v_prov_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_remetente_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Remetente ************************* */
async function delRemetente(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  let elements = document.getElementsByClassName('checkDelRemetente fa fa-check-circle');
  let v_remetente_id;
  for (let element of elements) {
    v_remetente_id = element.attributes.value.value;

    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_remetente_provocacao/delete/" + v_prov_id + "/" + v_remetente_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
  ...
  Swal.fire({
    icon: "success",
    title: "Remetente(s) removido(s) com sucesso.",
    showConfirmButton: false,
    timer: 1500,
  }).then(function () {
    listRemetente(v_prov_id);
...
/******************** Selecionar e descelecionar ************************** */
function checkDelRemetente(v_obj) {
  if (v_obj.className == 'checkDelRemetente far fa-circle') {
    v_obj.className = 'checkDelRemetente fa fa-check-circle';
  } else {
    v_obj.className = 'checkDelRemetente far fa-circle';
  }
}

function selRemetente(v_id) {
  $('#modalRemetente').modal('show');
  selectRemetente(v_id);
  sessionStorage.setItem("v_AddPes", 'remetente');
}
...
async function addPalChave(v_prov_id, v_dpalChave_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delPalChave(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_provocacao/delete/" + v_prov_id + "/" + v_pchave_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addRequerente(v_prov_id, v_requerente_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Requerente ************************* */
async function delRequerente(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_provocacao/delete/" + v_requerente_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Tema ***************************** */
/********************* Adicionar Tema ************************* */
async function addTema(v_prov_id, v_tema_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Tema ************************* */
async function delTema(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_provocacao/delete/" + v_tema_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
function selTema(v_id) {
  ...
  selectTema(v_id, document.getElementById('procTema').value);
}
...
/************************** Termo ***************************** */
/********************* Adicionar Termo ************************* */
async function addTermo(v_prov_id, v_termo_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Termo ************************* */
async function delTermo(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_provocacao/delete/" + v_termo_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Freguesia ***************************** */
/********************* Adicionar Freguesia ************************* */
async function addFreguesia(v_prov_id, v_freg_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Freguesia ************************* */
async function delFreguesia(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_provocacao/delete/" + v_freg_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Comarca ***************************** */
/********************* Adicionar Comarca ************************* */
async function addComarca(v_prov_id, v_coma_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Comarca ************************* */
async function delComarca(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_provocacao/delete/" + v_coma_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Capitania ***************************** */
/********************* Adicionar Capitania ************************* */
async function addCapitania(v_prov_id, v_cap_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Capitania ************************* */
async function delCapitania(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_provocacao/delete/" + v_coma_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addProvocacao(v_prov_id, v_provocacao_id) {

  var objResp = {
    provocacao_mais_recente: v_provocacao_id,
    provocacao_mais_antiga: v_prov_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Provocação ************************* */
async function delProvocacao(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_provocacao/delete/" + v_procacao_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
function selProvocacao(v_id) {
  $('#modalProvocacao').modal('show');
  selectProvocacao(v_id, document.getElementById('procProvocacao').value);
}

/************************** Mandado ***************************** */
/********************* Adicionar Mandado ************************* */
async function addMandado(v_prov_id, v_mandado_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Mandado ************************* */
async function delMandado(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_provocacao/delete/" + v_mandado_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Consulta ***************************** */
/********************* Adicionar Consulta ************************* */
async function addConsulta(v_prov_id, v_consulta_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Consulta ************************* */
async function delConsulta(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_provocacao/delete/" + v_consulta_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Ultramar ***************************** */
/********************* Adicionar Ultramar ************************* */
async function addUltramar(v_prov_id, v_ultramar_id) {
...
/********************* Retirar Ultramar ************************* */
async function delUltramar(v_prov_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_provocacao/delete/" + v_ultramar_id + "/" + v_prov_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Resposta ***************************** */
/********************* Adicionar Resposta ************************* */
async function addResposta(v_provoca_id, v_resp_id) {

  var objResp = {
    provocacao: v_provoca_id,
    resposta: v_resp_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
...
/********************* Retirar Resposta ************************* */
async function delResposta(v_provoca_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/delete/" + v_provoca_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        Swal.fire({
          ...
          let v_tipoAdd = sessionStorage.getItem("v_AddPes");
          if (v_tipoAdd == 'requerente') {
            addRequerente(sessionStorage.getItem("v_idProv"), pessoaId);
          } else {
            if (v_tipoAdd == 'pesCitada') {
              addPesCitada(sessionStorage.getItem("v_idProv"), pessoaId)
            } else {
              if (v_tipoAdd == 'remetente') {
                addRemetente(sessionStorage.getItem("v_idProv"), pessoaId)
              }
            }
          }
        });
...
async function delProv() {
  Swal.fire({
    icon: "error",
    title: "Tem a certeza que quer eliminar esta provocação?",
    ...
    if (result.isConfirmed) {
      ...
      myHeaders.append("token", await getToken());
      $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/provocacao/delete/" + sessionStorage.getItem("v_idProv"),
        type: "DELETE",
        headers: { "token": await getToken() },
        dataType: "json",
        success: function (result) {
          Swal.fire({
            icon: "success",
            title: "Provocação eliminada com sucesso.",
            showConfirmButton: false,
            timer: 1500,
          }).then(function () {
            window.location = "provocacao.html";
        ...
        error: function (err) {
          ...
          Swal.fire({
            icon: "error",
            title: "Erro ao elimiar a provocação.",
            text: "Esta provocação encontra-se associada a um registo",
            showConfirmButton: false,
            timer: 1500,
          });
        }
