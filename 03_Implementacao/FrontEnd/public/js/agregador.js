...
async function getAllAgregador(pageNum, rowsPage) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/oficio_agregador/all/" + pageNum + "/" + rowsPage;
    ...
    await fetch(myRequest).then(async function (response) {
        if (!response.ok) {
            ...
            for (const Agregador of Agregadores) {
                tabela += `<tr>
                                    <td scope="row"><i class="fa fa-pen-fancy" style="cursor:pointer;" onclick="getAgregador(`+ Agregador.pk_oficio_agregador + `)"></i></td>
...
async function search(v_search) {
    ...
    if (v_search == null || v_search == '') {
        ...
        urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/oficio_agregador/search/" + v_search;
...
async function getAgregador(v_aregadorId) {
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/oficio_agregador/record/" + v_aregadorId;
    var myHeaders = new Headers();
    myHeaders.append("token", await getAdminToken());
...
async function addAgregador() {
    ...
    if (errForm != "") {
        ...
        myHeaders.append("token", await getAdminToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/oficio_agregador/insert",
            type: "POST",
...
async function saveAgregador() {
    ...
    if (errForm != "") {
        ...
        myHeaders.append("token", await getAdminToken());
        $.ajax({
            url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/oficio_agregador/update/" + document.getElementById("agregadorId").value,
            type: "PUT",
            headers: { "token": await getAdminToken() },
            data: json,
            dataType: "json",
...
function delAgregador() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            myHeaders.append("token", await getAdminToken());
            $.ajax({
                url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/oficio_agregador/delete/" + document.getElementById("agregadorId").value,
                type: "DELETE",
                headers: { "token": await getAdminToken() },
                dataType: "json",
                success: function (result) {
