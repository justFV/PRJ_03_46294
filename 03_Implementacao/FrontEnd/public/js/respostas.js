function getAno(v_valor) {
  document.getElementById("anoResp").value = v_valor.substr(6);
}
...
async function filtrar(vsearch) {
    ...
    if (proc == 'all' && pessoa == 'all' && tema == '0' && palChave == '0') {
        ...
        const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/search/" + proc + "/" + pessoa + "/" + tema + "/" + palChave;
        console.log(urlBase);
              ...
              Respostas = await response.json();
                ...
                for (const resposta of Respostas) {
                    tabela += `<tr>
                                    ...
                                    <td scope="row">` + resposta.referencia_documental + `</td>
                                    <td scope="row">` + resposta.registro + `</td>
                                    <td scope="row">` + resposta.pescita + `</td>
                                    <td scope="row">` + resposta.pesreq + `</td>
                                    <td scope="row">` + resposta.palcha + `</td>
                                    <td scope="row">` + resposta.tema + `</td>
                                    <td>` + resposta.ano_resposta + `</td>
                               </tr>`;
                    if (conta >= 10) {
                        break;
                    }
...
async function getAllRespostas(pageNum, rowsPage) {
  ...
  urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/all/" + pageNum + "/" + rowsPage;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      console.log("Erro ao devolver a lista de respostas");
    } else {
      objRes = await response.json();
      respostas = objRes.allPalResposta;
      console.log(respostas);
      let tabela = '<table class="table">' +
        `<tr>` +
        `<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Resumo</th><th scope="col">Referência documental</th>` +
        `<th scope="col">Registro</th><th scope="col">Pessoas<br>Citadas</th><th scope="col">Requerente</th><th scope="col">Palavra<br>Chave</th><th>Tema</th><th>Ano Resposta</th>` +
        `</tr>`;
      for (const resposta of respostas) {
        tabela += `<tr>
                                <td scope="row"><i class="fa fa-pen-fancy" style="cursor:pointer;" onclick="editResposta(`+ resposta.pk_resposta + `)"></i></td>
      ...
      let numPage = Math.ceil(objRes.rowcount / rowsPage);
      if (pageNum != 1) {
        tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllRespostas(1,` + rowsPage + `)" style="color: white;">Inicio</a></span></li>`;
        if (pageNum > 5) {
      ...
      for (let i = 1; i <= numPage; i++) {
        if (i == pageNum) {
          ...
          if (i > pageNum - 5 && i < pageNum + 5) {
            tabela += ` <li class="page-item"><a class="page-link" href="javascript:getAllRespostas(` + i + `,` + rowsPage + `)">` + i + `</a></li>`;
          }
        }
      }
      if (pageNum != numPage) {
        ...
        tabela += `<li class="page-item active"><span class="page-link" ><a href="javascript:getAllRespostas(` + numPage + `,` + rowsPage + `)" style="color: white;">Fim</a></span></li>`;
      }
      tabela += `</ul> `;
      document.getElementById("TableResposta").innerHTML = tabela;
    }
...
async function search(v_search) {
  if (v_search == null || v_search == '') {
    ...
    const tema = document.getElementById("selFilTema").value;
    const palChave = document.getElementById("selFilPalCha").value;
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/search/" + v_search + "/" + pessoa + "/" + tema + "/" + palChave;
    ...
    await fetch(myRequest).then(async function (response) {
      if (!response.ok) {
        ...
        let tabela = '<table class="table">' +
          '<tr>' +
          '<th scope="col">Editar</th><th scope="col">Nº</th><th scope="col">Data Resposta</th><th scope="col">Resumo</th><th scope="col">Referência documental</th>' +
          '<th scope="col">Fólio/Página</th><th scope="col">Nova Ordem / Não Cumprimento</th><th scope="col">Tipologia Diplomática</th><th scope="col">Registro</th><th>Ano Resposta</th>' +
          '</tr>';
        var conta = 0;
        for (const resposta of respostas) {
          ...
          tabela += `<tr>
                                <td scope="row"><i class="fa fa-pen-fancy" style="cursor:pointer;" onclick="editResposta(`+ resposta.pk_resposta + `)"></i></td>
                                <td scope="row">` + resposta.pk_resposta + `</td>
                                <td scope="row">` + resposta.data_resposta + `</td><td scope="row">` + resposta.resumo + `</td>
                                <td scope="row">`+ resposta.referencia_documental + `</td>
                                <td scope="row">` + resposta.folio_pagina + `</td>
                                <td scope="row">`+ resposta.nova_ordem_nao_cumprimento + `</td>
                                <td scope="row">`+ resposta.tipologia_diplomatica + `</td>
                                <td scope="row">`+ resposta.registro + `</td><td>` + resposta.ano_resposta + `</td>
                            </tr>`
          if (conta >= 10) {
            break;
          }
        ...
        if (conta >= 10) {
          ...
          if (conta == 0) {
            tabela += '<tr><td colspan="8"><h4>Esta pesquisa não devolveu valores.<br>Por favor faça uma nova.</h4></td></tr>'
          }
        }
        tabela += `</table>`;
        document.getElementById("TableResposta").innerHTML = tabela;
...
async function getResposta(v_respId) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/record/" + v_respId;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      document.getElementById("resObs").value = resposta.resumo;
      document.getElementById("anoResp").value = resposta.ano_resposta;
      document.getElementById("novaOrd").value = resposta.nova_ordem_nao_cumprimento;
      document.getElementById("impressos").value = resposta.impressos;
      document.getElementById("datResp").value = resposta.data_resposta;
      document.getElementById("folioPagina").value = resposta.folio_pagina;
      document.getElementById("IdResp").innerHTML = 'Resposta Nº ' + v_idResp;
      if (resposta.anoPost == "Y") {
        document.getElementById("anoPos").checked = true;
      } else {
        document.getElementById("anoPos").checked = false;
      }
      if (resposta.anoPost == "N") {
        document.getElementById("anoAnt").checked = true;
      } else {
        document.getElementById("anoAnt").checked = false;
      }
      getRefDoc(v_respId, resposta.referencia_documental);
      listRefDoc('');
      listRegistroAdd(resposta.registro, '');
      // getAllTipoDiplo(resposta.tipologia_diplomatica);
      listSecEstado(v_idResp);
      listInstado(v_idResp);
      listDestinatario(v_idResp);
      listPalavraChave(v_idResp);
      listRequerente(v_idResp);
      listTema(v_idResp);
      listProvocacao(v_idResp);
      listMandado(v_idResp);
      listConsulta(v_idResp);
      listUltramar(v_idResp);
      listResposta(v_idResp);
      listTermo(v_idResp);
      listFreguesia(v_idResp);
      listComarca(v_idResp);
      listCapitania(v_idResp);
...
async function listRegistroAdd(v_id, v_search) {
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let tableRegistros = '<div class="scroll"><table id="tableRegistro" class="ListPopUpModal">';
      let conta = 0
...
async function getAllTipoDiplo(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/tipologia_diplomatica/all";
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let tipoDiplomas = await response.json();
      console.log(tipoDiplomas);
      let selectbox = '<label for="tipoDiploSel" >Tipologia Diplomática:</label>' +
        '<select class="form-control" id="tipoDiploSel" name="tipoDiploSel" placeholder="Tipologia Diplomática">' +
        '<option>&nbsp;</option>';
      let selected = '';
      for (const tipoDiploma of tipoDiplomas) {
        if (v_id == tipoDiploma.pk_tipologia) {
          selected = 'selected="selected"';
        } else {
          selected = '';
        }
        selectbox += '<option value="' + tipoDiploma.pk_tipologia + '" ' + selected + '>' + tipoDiploma.nome + '</option>'


      }
      selectbox += `</select>`;
      document.getElementById("tipoDiplo").innerHTML = selectbox;
...
async function listSecEstado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_resposta/allResposta/" + v_id;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectSecEstado(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
/****************** Instado ******************* */
/******************** Devolve lista dos instados já introduzidos *************************** */
async function listInstado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_instado_resposta/allRespostas/" + v_id;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      console.log("Erro ao devolver a lista de Instados");
    } else {
      let Instados = await response.json();
      console.log(Instados);
      let tableInstado = '<div class="row" ><div class="column listTabNome" align="left"><label for="tableListInstado" >Pessoas Citadas:</label>' +
        '</div><div class="column listTabBotao" align="right"><a href="javascript:selInstado(' + v_id + ')"><img src="../img/addLine.png" width="35"></a> ' +
        '<a href="javascript:delInstado(' + v_id + ')"><img src="../img/delList.png" width="35"></a></div></div>' +
        '<table id="tableListInstado" class="listPopUp">';

      for (const Instado of Instados) {

        tableInstado += '<tr><td class="checkDel"><i class="checkDelInstado far fa-circle" style="color:red;" onclick="checkDelInstado(this);" value="' + Instado.id + '"></i></td><td>' + Instado.nome + '</td></tr>'


      }
      tableInstado += `</table>`;
      document.getElementById("instado").innerHTML = tableInstado;
      console.log(tableInstado)
    }
  })
}
/*********************** Devolve lista dos instados que ainda não foram escolhidos ************************************** */
async function selectInstado(v_id, v_nomePesq) {
  if (v_nomePesq == null) {
    v_nomePesq = 'all';
  }
  if (v_nomePesq == '') {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_instado_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let listInstados = await response.json();
      console.log(listInstados);
      let tableInstado = '<table id="tableSelInstado" class="ListPopUpModal">';
      let v_conta = 0;
      for (const listInstado of listInstados) {

        tableInstado += '<tr><td><a href="javascript:addInstado(' + v_id + ',' + listInstado.id + ')">' + listInstado.nome + '</a></td>' +
          '<td> <span type="button" onclick="getPessoaInfo(' + listInstado.id + ')">' +
      ...
      if (v_conta >= 10) {
        tableInstado += '<tr><td>Esta pesquisa devolveu muitos valores.<br>Por afine a sua pesquisa para devolver menos valores.</td></tr>'
      }
      tableInstado += `</table>`;
      document.getElementById("tabmodalInstado").innerHTML = tableInstado;
    }
  })
}
...
/****************** Destinatário ******************* */
/******************** Devolve lista dos destinatários já introduzidos *************************** */
async function listDestinatario(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_destinatario_resposta/allRespostas/" + v_id;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      console.log("Erro ao devolver a lista de Destinatários");
    } else {
      let Destinatarios = await response.json();
      console.log(Destinatarios);
      let tableDestinatario = '<div class="row" ><div class="column listTabNome" align="left"><label for="tableListDestinatario" >Destinatários:</label>' +
        '</div><div class="column listTabBotao" align="right"><a href="javascript:selDestinatario(' + v_id + ')"><img src="../img/addLine.png" width="35"></a> ' +
        '<a href="javascript:delDestinatario(' + v_id + ')"><img src="../img/delList.png" width="35"></a></div></div>' +
        '<table id="tableListDestinatario" class="listPopUp">';

      for (const Destinatario of Destinatarios) {

        tableDestinatario += '<tr><td class="checkDel"><i class="checkDelDestinatario far fa-circle" style="color:red;" onclick="checkDelDestinatario(this);" value="' + Destinatario.id + '"></i></td><td>' + Destinatario.nome + '</td></tr>'


      }
      tableDestinatario += `</table>`;
      document.getElementById("destinatario").innerHTML = tableDestinatario;
      console.log(tableDestinatario)
    }
  })
}

/*********************** Devolve lista dos destinatários que ainda não foram escolhidos ************************************** */
async function selectDestinatario(v_id, v_nomePesq) {
  if (v_nomePesq == null) {
    v_nomePesq = 'all';
  }
  if (v_nomePesq == '') {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_destinatario_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  let texto = "";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let listDestinatarios = await response.json();
      console.log(listDestinatarios);
      let tableDestinatario = '<table id="tableSelDestinatario" class="ListPopUpModal">';
      let conta = 1;
      for (const listDestinatario of listDestinatarios) {

        tableDestinatario += '<tr><td><a href="javascript:addDestinatario(' + v_id + ',' + listDestinatario.id + ')">' + listDestinatario.nome + '</a></td>' +
          '<td> <span type="button" onclick="getPessoaInfo(' + listDestinatario.id + ')">' +
          '<img src="../img/info.png" width="20px"></span></td></tr>'
      ...
      if (conta >= 10) {
        tableDestinatario += '<tr><td>Esta pesquisa devolveu muitos valores.<br>Por afine a sua pesquisa para devolver menos valores.</td></tr>'
      }
      tableDestinatario += `</table>`;
      document.getElementById("tabmodalDestinatario").innerHTML = tableDestinatario;
    }
  })
}
...
async function listPalavraChave(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_resposta/allResposta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectPalChave(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listRequerente(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_resposta/allResposta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectRequerente(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listTema(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_resposta/allResposta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectTema(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listTermo(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_resposta/allResposta/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectTermo(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listFreguesia(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_resposta/allRespostas/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectFreguesia(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listComarca(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_resposta/allResposta/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectComarca(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listCapitania(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_resposta/allResposta/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectCapitania(v_id, v_nomePesq) {
  ...
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function listProvocacao(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/allResposta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectProvocacao(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listMandado(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_resposta/allResposta/" + v_id + "/all";
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectMandado(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function getMandadoInfo(manId) {
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      sessionStorage.setItem("v_idMan", manId);
...
async function listConsulta(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/allRespostas/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
    ...
    if (!response.ok) {
      ...
      for (const consulta of consultas) {
        ...
        tableConsulta += '<tr><td class="checkDel"><i class="checkDelConsulta far fa-circle" style="color:red;" onclick="checkDelConsulta(this);" value="' + consulta.id + '"></i></td>' +
          ...
          '<td>' + consulta.sumula + ' (' + consulta.ano_consulta + ')</td></tr>'


      }
      tableConsulta += `</table>`;
      document.getElementById("consulta").innerHTML = tableConsulta;
      console.log(tableConsulta)
    }
  })
}
...
async function selectConsulta(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
...
async function listUltramar(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_resposta/allResposta/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
...
async function selectUltramar(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    ...
    await fetch(myRequest).then(async function (response) {
      if (!response.ok) {
        ...
        for (const listUltramar of listUltramares) {
          ...
          tableUltramar += '<tr><td><a href="javascript:addUltramar(' + v_id + ',' + listUltramar.id + ')"><b>Nº:' + listUltramar.id + '</b>-' + listUltramar.resumo + ' (' + listUltramar.ano + ')</a></td>' +
...
async function getUltramarInfo(UltId) {
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      document.getElementById("anoUlt").innerHTML = ultramar.ano;
      document.getElementById("autUlt").innerHTML = ultramar.autoridade;
      document.getElementById("datUlt").innerHTML = ultramar.data;
...
async function listResposta(v_id) {
  const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_resposta_resposta/allRespAntiga/" + v_id;
  var myHeaders = new Headers();
  myHeaders.append("token", await getToken());
  ...
  await fetch(myRequest).then(async function (response) {
    if (!response.ok) {
      ...
      let tableResposta = '<div class="row" ><div class="column listTabNome" align="left"><label for="tableResposta" >Resposta:</label>' +
        ...
        '<a href="javascript:delRespostaCheck(' + v_id + ')"><img src="../img/delList.png" width="35"></a></div></div>' +
      ...
      for (const resposta of Respostas) {
        ...
        tableResposta += '<tr><td class="checkDel"><i class="checkDelResposta far fa-circle" style="color:red;" onclick="checkDelResposta(this);" value="' + resposta.id + '"></i></td>' +
          ...
          '<td>' + resposta.resumo + ' (' + resposta.ano + ')</td></tr>'


      }
      tableResposta += `</table>`;
      document.getElementById("resposta").innerHTML = tableResposta;
      console.log(tableResposta)
    }
  })
}
...
async function selectResposta(v_id, v_nomePesq) {
  ...
  if (v_nomePesq == 'all' &&
    ...
    var json = JSON.stringify(objResp);
    const urlBase = await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_resposta_resposta/notInResposta/" + v_id + "/" + v_nomePesq;
    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    ...
    await fetch(myRequest).then(async function (response) {
      if (!response.ok) {
        ...
        for (const listResposta of listRespostas) {
          ...
          tableResposta += '<tr><td><a href="javascript:addResposta(' + v_id + ',' + listResposta.id + ')"><b>Nº:' + listResposta.id + '</b>-' + listResposta.resumo + ' (' + listResposta.ano + ')</a></td>' +
...
async function getPessoaInfo(pesId) {
  ...
  if (v_tipoAdd == 'requerente') {
    ...
    if (v_tipoAdd == 'destinatario') {
      $('#modalDestinatario').modal('hide');
    } else {
      if (v_tipoAdd == 'instado') {
        $('#modalInstado').modal('hide');
      }
    }
  }
  $('#infoPes').modal('show');
...
async function selectMarEco(v_nomePesq) {
  ...
  $('#criarPes').modal('hide'); $('#modalMarEco').modal('show');
  if (v_nomePesq == null) {
    v_nomePesq = 'all';
  }
  if (v_nomePesq == '') {
...
function limparFiltro() {
  ...
  selectProvocacao(sessionStorage.getItem("v_idResp"), document.getElementById('procProvocacao').value);
}
...
function limparFiltroMan() {
  ...
  selectMandado(sessionStorage.getItem("v_idResp"), document.getElementById('procMandado').value);
}
...
function limparFiltroCon() {
  ...
  selectConsulta(sessionStorage.getItem("v_idResp"), document.getElementById('procConsulta').value);
}
...
function limparFiltroUltra() {
  ...
  selectUltramar(sessionStorage.getItem("v_idResp"), document.getElementById('procUltramar').value);
}
...
function limparFiltroRes() {
  ...
  selectResposta(sessionStorage.getItem("v_idResp"), document.getElementById('procResposta').value);
}
...
async function salvar() {
  ...
  if (document.getElementById("resObs").value == "") {
    errForm += "Resumo é obrigatório.<br>";
  }
  if (document.getElementById("anoResp").value == "") {
  ...
  if (errForm != "") {
    ...
    let v_idResp = sessionStorage.getItem("v_idResp");
    let v_anoPos = '';
    ...
    var objResp = {
      data_resposta: document.getElementById("datResp").value,
      resumo: document.getElementById("resObs").value,
      nova_ordem_nao_cumprimento: document.getElementById("novaOrd").value,
      registro: document.getElementById("registroAddValue").value,
      ano_resposta: document.getElementById("anoResp").value,
      referencia_documental: document.getElementById("ref_docValue").value,
      impressos: document.getElementById("impressos").value,
      anoPost: v_anoPos,
    ...
    $.ajax({
      ...
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/update/" + v_idResp,
      type: "PUT",
      headers: { "token": await getToken() },
      data: json,
      dataType: "json",
      success: function (result) {
        sessionStorage.setItem("v_idResp", result);
        Swal.fire({
          icon: "success",
          title: "Resposta guardada com sucesso",
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
      ...
      error: function (err) {
        ...
        Swal.fire({
          icon: "error",
          title: "Erro ao alterar a resposta",
          showConfirmButton: false,
          timer: 1500,
        });
      }
...
// função de criação de nova reposta
async function addNovaResposta(v_id) {
  var errForm = "";
  // validação do preenchimento dos campos
  if (document.getElementById("resObs").value == "") {
    errForm += "Resumo é obrigatório.<br>";
  ...
  if (document.getElementById("registroAddValue").value == "") {
    errForm += "Registro é obrigatório.<br>";
  }
  if (document.getElementById("anoResp").value == "") {
    errForm += "Ano é obrigatório.<br>";
  }
  ...
  if (errForm != "") {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/insert",
      type: "POST",
      headers: { "token": await getToken() },
      data: json,
      dataType: "json",
      success: function (result) {
        ...
        Swal.fire({
          icon: "success",
          title: "Resposta criada com sucesso",
          showConfirmButton: false,
          timer: 1500,
        }).then(function () {
          sessionStorage.setItem("v_idResp", result);
...
async function addSecEstado(v_resp_id, v_secEstado_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delSecEstado(v_resp_id) {
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_secretario_conselho_resposta/delete/" + v_secEstado_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/********************* Adicionar Instado ************************* */
async function addInstado(v_resp_id, v_instado_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
    instado: v_instado_id,
    resposta: v_resp_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_instado_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Instado ************************* */
async function delInstado(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  let elements = document.getElementsByClassName('checkDelInstado fa fa-check-circle');
  let v_instado_id;
  for (let element of elements) {
    v_instado_id = element.attributes.value.value;

    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_instado_resposta/delete/" + v_resp_id + "/" + v_instado_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/******************** Selecionar e descelecionar ************************** */
function checkDelInstado(v_obj) {
  if (v_obj.className == 'checkDelInstado far fa-circle') {
    v_obj.className = 'checkDelInstado fa fa-check-circle';
  } else {
    v_obj.className = 'checkDelInstado far fa-circle';
  }
}


function selInstado(v_id) {
  $('#modalInstado').modal('show');
  sessionStorage.setItem("v_AddPes", 'instado');
  selectInstado(v_id);
}
...
async function addDestinatario(v_resp_id, v_destinatario_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
    destinatario: v_destinatario_id,
    resposta: v_resp_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_destinatario_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
    success: function (result) {
      Swal.fire({
        icon: "success",
        title: "Destinatário adicionado com sucesso.",
        showConfirmButton: false,
        timer: 1500,
      }).then(function () {
        $('#modalDestinatario').modal('hide');
        listDestinatario(v_resp_id);
      });
...
/********************* Retirar Destinatário ************************* */
async function delDestinatario(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  let elements = document.getElementsByClassName('checkDelDestinatario fa fa-check-circle');
  let v_destinatario_id;
  for (let element of elements) {
    v_destinatario_id = element.attributes.value.value;

    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_destinatario_resposta/delete/" + v_resp_id + "/" + v_destinatario_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
  ...
  Swal.fire({
    icon: "success",
    title: "Destinatário(s) removido(s) com sucesso.",
    showConfirmButton: false,
    timer: 1500,
  }).then(function () {
    listDestinatario(v_resp_id);
...
/******************** Selecionar e descelecionar ************************** */
function checkDelDestinatario(v_obj) {
  if (v_obj.className == 'checkDelDestinatario far fa-circle') {
    v_obj.className = 'checkDelDestinatario fa fa-check-circle';
  } else {
    v_obj.className = 'checkDelDestinatario far fa-circle';
  }
}


function selDestinatario(v_id) {
  $('#modalDestinatario').modal('show');
  sessionStorage.setItem("v_AddPes", 'destinatario');
  selectDestinatario(v_id);
}
...
/************************** Termo ***************************** */
/********************* Adicionar Termo ************************* */
async function addTermo(v_res_id, v_termo_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Termo ************************* */
async function delTermo(v_res_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_termo_resposta/delete/" + v_termo_id + "/" + v_res_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Freguesia ***************************** */
/********************* Adicionar Freguesia ************************* */
async function addFreguesia(v_res_id, v_freg_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Freguesia ************************* */
async function delFreguesia(v_res_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_freguesia_resposta/delete/" + v_freg_id + "/" + v_res_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Comarca ***************************** */
/********************* Adicionar Comarca ************************* */
async function addComarca(v_res_id, v_coma_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Comarca ************************* */
async function delComarca(v_res_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_comarca_resposta/delete/" + v_coma_id + "/" + v_res_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Capitania ***************************** */
/********************* Adicionar Capitania ************************* */
async function addCapitania(v_res_id, v_cap_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Capitania ************************* */
async function delCapitania(v_res_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_capitania_resposta/delete/" + v_coma_id + "/" + v_res_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addPalChave(v_resp_id, v_dpalChave_id) {
  //Se não houver erro no preenchimento então chama o serviço de registo
  var objResp = {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
async function delPalChave(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_pchave_resposta/delete/" + v_resp_id + "/" + v_destinatario_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addRequerente(v_resp_id, v_requerente_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Requerente ************************* */
async function delRequerente(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_requerente_resposta/delete/" + v_requerente_id + "/" + v_resp_id + "/1",
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Tema ***************************** */
/********************* Adicionar Tema ************************* */
async function addTema(v_resp_id, v_tema_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Tema ************************* */
async function delTema(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_tema_resposta/delete/" + v_tema_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
async function addProvocacao(v_resp_id, v_provocacao_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Provocação ************************* */
async function delProvocacao(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_provocacao_resposta/delete/" + v_procacao_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Mandado ***************************** */
/********************* Adicionar Mandado ************************* */
async function addMandado(v_resp_id, v_mandado_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Mandado ************************* */
async function delMandado(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_mandado_resposta/delete/" + v_mandado_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Consulta ***************************** */
/********************* Adicionar Consulta ************************* */
async function addConsulta(v_resp_id, v_consulta_id) {
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Consulta ************************* */
async function delConsulta(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  ...
  for (let element of elements) {
    ...
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_consulta_resposta/delete/" + v_consulta_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Ultramar ***************************** */
/********************* Adicionar Ultramar ************************* */
async function addUltramar(v_resp_id, v_ultramar_id) {

  var objResp = {
    ultramar: v_ultramar_id,
    resposta: v_resp_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
...
/********************* Retirar Ultramar ************************* */
async function delUltramar(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  let elements = document.getElementsByClassName('checkDelUltramar fa fa-check-circle');
  let v_ultramar_id;
  for (let element of elements) {
    v_ultramar_id = element.attributes.value.value;

    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_ultramar_resposta/delete/" + v_ultramar_id + "/" + v_resp_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
...
/************************** Resposta ***************************** */
/********************* Adicionar Resposta ************************* */
async function addResposta(v_resp_id, v_resposta_id) {

  var objResp = {
    resposta_mais_recente: v_resposta_id,
    resposta_mais_antiga: v_resp_id
  };
  var exit = "no";
  var json = JSON.stringify(objResp);
  ...
  myHeaders.append("token", await getToken());
  $.ajax({
    url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_resposta_resposta/insert",
    type: "POST",
    headers: { "token": await getToken() },
    data: json,
    dataType: "json",
...
/********************* Retirar Resposta ************************* */
async function delRespostaCheck(v_resp_id) {
  // vai percorrer todos os item da página que utilizem a class checked
  // Se checked e clicar eliminar remove da lista os selecionados
  let elements = document.getElementsByClassName('checkDelResposta fa fa-check-circle');
  let v_resposta_id;
  for (let element of elements) {
    v_resposta_id = element.attributes.value.value;

    var myHeaders = new Headers();
    myHeaders.append("token", await getToken());
    $.ajax({
      url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/relac_resposta_resposta/delete/" + v_resp_id + "/" + v_resposta_id,
      type: "DELETE",
      headers: { "token": await getToken() },
      data: "",
      dataType: "json"
  ...
  Swal.fire({
    icon: "success",
    title: "Resposta(as) removida(s) com sucesso.",
    showConfirmButton: false,
    timer: 1500,
  }).then(function () {
    listResposta(v_resp_id);
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        Swal.fire({
          ...
          let v_tipoAdd = sessionStorage.getItem("v_AddPes");
          if (v_tipoAdd == 'requerente') {
            addRequerente(sessionStorage.getItem("v_idResp"), pessoaId);
          } else {
            if (v_tipoAdd == 'destinatario') {
              addDestinatario(sessionStorage.getItem("v_idResp"), pessoaId);
            } else {
              if (v_tipoAdd == 'instado') {
                addInstado(sessionStorage.getItem("v_idResp"), pessoaId)
              }
            }
          }
        });
...
async function delResposta() {
  Swal.fire({
    icon: "error",
    title: "Tem a certeza que quer eliminar esta resposta?",
    ...
    if (result.isConfirmed) {
      ...
      myHeaders.append("token", await getToken());
      $.ajax({
        url: await getUrlPayaraDireto() + "/CircPeticionario/webresources/resposta/delete/" + sessionStorage.getItem("v_idResp"),
        type: "DELETE",
        headers: { "token": await getToken() },
        dataType: "json",
        success: function (result) {
          ...
          Swal.fire({
            icon: "success",
            title: "Resposta eliminada com sucesso.",
            showConfirmButton: false,
            timer: 1500,
          }).then(function () {
            window.location = "resposta.html";
        ...
        error: function (err) {
          ...
          Swal.fire({
            icon: "error",
            title: "Erro ao elimiar a resposta.",
            text: "Esta resposta encontra-se associada a um registo",
            showConfirmButton: false,
            timer: 1500,
          });
          /*  .then(function () {
