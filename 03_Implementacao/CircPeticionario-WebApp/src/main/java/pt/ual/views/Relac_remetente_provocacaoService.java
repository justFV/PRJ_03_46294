 ...
 import pt.ual.dao.Relac_remetente_provocacao;
 import pt.ual.utils.Utils;

 @Path("relac_remetente_provocacao")
 public class Relac_remetente_provocacaoService
 {
   @GET
   ...
   public Response allProvocacao(@PathParam("provocacao") int provocacao, @Context HttpHeaders headers, @HeaderParam("token") String authKey) throws Exception {
     ...
     Relac_remetente_provocacao ptr = new Relac_remetente_provocacao();
   ...
   @GET
   @Path("/allRemetente/{remetente}/{nomePesq}")
   @Produces({"application/json"})
   public Response allRemetente(@PathParam("remetente") int remetente, @PathParam("nomePesq") String nomePesq, @Context HttpHeaders headers, @HeaderParam("token") String authKey) throws Exception {
     boolean result = false;
     Response.ResponseBuilder response = null;
     Relac_remetente_provocacao ptr = new Relac_remetente_provocacao();
     String token = (new Utils()).getToken();
   ...
   @GET
   @Path("/notInRemetente/{remetente}/{nomePesq}")
   @Produces({"application/json"})
   public Response notInConsulta(@PathParam("remetente") int remetente, @PathParam("nomePesq") String nomePesq, @Context HttpHeaders headers, @HeaderParam("token") String authKey) throws Exception {
     boolean result = false;
     Response.ResponseBuilder response = null;
     Relac_remetente_provocacao ptr = new Relac_remetente_provocacao();
     String token = (new Utils()).getToken();
     ...
     try {
       if (authKey == null || !authKey.equals(token)) {
         ...
         response = Response.ok(ptr.notInRemetente()).header("content-type", "application/json; charset=UTF-8");
       } 
     } catch (Exception ex) {
       ex.printStackTrace();
       response = Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(Boolean.valueOf(result)).header("content-type", "application/json; charset=UTF-8");
   ...
   public Response insert(String data, @Context HttpHeaders headers, @HeaderParam("token") String authKey) throws Exception {
     ...
     try {
       if (authKey == null || !authKey.equals(token)) {
         ...
         Relac_remetente_provocacao ptr = (Relac_remetente_provocacao)(new ObjectMapper()).readValue(data, Relac_remetente_provocacao.class);
